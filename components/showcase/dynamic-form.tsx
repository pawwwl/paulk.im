...
function inferFieldType(header: string, samples: string[]): FieldType {
  ...
  if (
    /message|description|notes?|comments?|bio|about|summary|body|details?/.test(
      h,
    )
  )
    return "textarea";
  ...
  if (nonEmpty.some((s) => s.length > 200)) return "textarea";
  if (nonEmpty.every((s) => /^-?\d+(\.\d+)?$/.test(s.trim()))) return "number";
  if (nonEmpty.every((s) => looksLikeDate(s))) return "date";
  if (nonEmpty.every((s) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(s)))
    return "email";
  const uniqueValues = new Set(nonEmpty.map((s) => s.trim().toLowerCase()));
  if (uniqueValues.size < 10 && nonEmpty.length >= uniqueValues.size * 2)
    return "select";
  return "text";
}

function csvToTemplate(
  filename: string,
  headers: string[],
  rows: string[][],
): FormTemplate {
...
const thStyle =
  "px-3 py-2 text-left text-[rgba(255,255,255,0.3)] font-mono text-[10px] font-bold tracking-[0.1em] border-b border-[rgba(255,255,255,0.14)] whitespace-nowrap";

const tdStyle =
  "px-3 py-[9px] border-b border-[rgba(255,255,255,0.04)] max-w-[240px] overflow-hidden text-ellipsis whitespace-nowrap";
    ...
    >
      <div
        ...
        className={cn(
          ...
          isMobile
            ? "rounded-[14px_14px_0_0] px-4 pt-5 pb-8 w-full"
            : "rounded-[14px] p-7 w-130",
        )}
      >
              ...
              >
                {entries.map(([type, meta]) => (
                  <button
                    ...
                    className="bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.14)] rounded-[9px] px-3 py-2.75 cursor-pointer flex items-center gap-2.5 text-left transition-all duration-130"
                    onMouseEnter={(e) => {
...
const panelLabelStyle =
  "block text-[rgba(255,255,255,0.35)] font-mono text-[10px] tracking-[0.1em] mb-[5px]";

const panelInputStyle =
  "w-full bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.09)] rounded-[7px] px-[10px] py-[7px] text-[#e0e0e0] font-mono text-xs outline-none box-border transition-colors duration-150";
...
function PanelToggle({
  ...
  return (
    <div className="flex items-center justify-between mb-3">
      <span className="text-[rgba(255,255,255,0.55)] font-mono text-[12px]">
        {label}
      </span>
      <button
        onClick={() => onCh(!value)}
    ...
    <div
      className={cn(
        ...
        isMobile
          ...
          : "w-68 shrink-0 border-l border-[rgba(255,255,255,0.14)]",
      )}
    >
      {/* Header */}
      ...
      <div className="px-4.5 pt-1 pb-6 flex-1">
          ...
          <>
            ...
            <div className="mb-3">
              ...
              <textarea
                ...
                rows={4}
                onChange={(e) =>
                  upd({ content: e.target.value, label: e.target.value })
                }
        ...
        {!isLayout && (
          <>
            ...
            <PanelField
              ...
              onCh={(v) => upd({ helpText: v })}
              placeholder="Optional hint…"
            />
            <PanelToggle
              label="Required"
              value={field.required}
              onCh={(v) => upd({ required: v })}
            />
          </>
        )}
              ...
              {field.options.map((opt) => (
                <div key={opt.id} className="flex items-center gap-1.5">
                  <input
                    ...
                    onFocus={(e) =>
                      (e.target.style.borderColor = meta.color + "88")
                    }
                    onBlur={(e) =>
                      (e.target.style.borderColor = "rgba(255,255,255,0.09)")
                    }
                  ...
                  <button
                    ...
                    onMouseEnter={(e) => {
                      if (field.options.length > 1)
                        e.currentTarget.style.color = "#E85D7A";
                    }}
                    onMouseLeave={(e) => {
        ...
        {/* Text validation */}
        {["text", "email", "tel", "url", "password", "textarea"].includes(
          field.type,
        ) && (
          <>
          ...
          <>
            ...
            <div className="mb-3">
              ...
              <select
                ...
                onChange={(e) =>
                  upd({
                    relatedTemplateId: e.target.value,
                    relatedDisplayField: "",
                  })
                }
                className={cn(
            ...
            {field.relatedTemplateId &&
              (() => {
                const related = templates.find(
                  (t) => t.id === field.relatedTemplateId,
                );
                const displayableFields =
                  related?.fields.filter(
                    (f) =>
                      !["heading", "paragraph", "divider", "relation"].includes(
                        f.type,
                      ),
                  ) ?? [];
                return (
                  <div className="mb-3">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        upd({ relatedDisplayField: e.target.value })
                      }
                      className={cn(
                        panelInputStyle,
                        "appearance-none cursor-pointer",
...
function FieldCard({
  ...
  const isLayout = meta.category === "Layout";
  const linkedForm =
    field.type === "relation" && field.relatedTemplateId
      ? templates.find((t) => t.id === field.relatedTemplateId)
      : null;
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isDragOver &&
          "opacity-60 outline-2 outline-dashed outline-[rgba(77,150,217,0.3)] outline-offset-2",
        !selected && !isDragOver && "bg-[rgba(255,255,255,0.02)]",
        !selected &&
          isDragOver &&
          "border-[rgba(77,150,217,0.4)] bg-[rgba(77,150,217,0.05)]",
      )}
      style={
        selected
          ? {
    ...
    >
      ...
      {field.width !== "full" && !isLayout && (
        <div
          className="h-0.5"
          style={{
            background: `linear-gradient(to right, ${meta.color}55 ${field.width === "half" ? "50%" : "33.3%"}, rgba(255,255,255,0.04) 0%)`,
          }}
        />
      )}
    </div>
    ...
    <button
      ...
      className={cn(
        ...
        hov
          ...
          : "bg-transparent border-[rgba(255,255,255,0.14)] text-[rgba(255,255,255,0.2)]",
      )}
    >
      <IconPlus />
      Add field
...
function PreviewField({
  ...
  const inputBase =
    "w-full bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.1)] rounded-lg px-3 py-[9px] text-[#e0e0e0] font-mono text-[13px] outline-none box-border transition-colors duration-150";
  ...
  if (field.type === "relation") {
    ...
    const displayFields =
      related?.fields.filter(
        (f) =>
          !["heading", "paragraph", "divider", "relation"].includes(f.type),
      ) ?? [];
    const displayField = displayFields.find(
      (f) => f.id === field.relatedDisplayField,
    );
      ...
      {field.type === "textarea" ? (
        ...
        <div className="flex flex-col gap-2.5">
          {field.options.map((o) => (
            ...
            >
              ...
              />
              <span className="text-[#d0d0d0] font-mono text-[12px]">
                {o.label}
              </span>
            </label>
          ))}
        ...
        <div>
          ...
          <div className="flex justify-between mt-1.25">
            <span className="text-[rgba(255,255,255,0.25)] font-mono text-[10px]">
              {field.min}
            </span>
            <span
      ...
      >
        ...
        <div
          ...
          className={cn(
            ...
            dragging
              ? "border-[#5AAD6B] bg-[rgba(90,173,107,0.14)]"
              : loading
        ...
        >
          {loading ? (
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-[rgba(255,255,255,0.35)] font-mono text-[12px]">
                Processing…
              </p>
            </div>
          ) : (
            <>
              <div
                className={cn(
                  ...
                  dragging
                    ? "bg-[rgba(90,173,107,0.15)] text-[#5AAD6B]"
                    : "bg-[rgba(255,255,255,0.05)] text-[rgba(255,255,255,0.35)]",
                )}
              >
                <IconUpload />
              </div>
              ...
              <div className="flex items-center justify-center gap-2">
                ...
                <span className="text-[rgba(255,255,255,0.3)] font-mono text-[10px]">
                  max 5 MB
                </span>
              </div>
            </>
...
function ConfirmDeleteOverlay({
  ...
  return (
    ...
    >
      ...
      >
        <p className="text-[#e0e0e0] font-mono text-[13px] font-bold mb-1.5">
          Confirm delete
        </p>
        <p className="text-[rgba(255,255,255,0.4)] font-mono text-[11px] leading-[1.6] mb-5">
          {message}
        </p>
...
export function DynamicFormBuilder() {
  ...
  const [mode, setMode] = useState<"build" | "preview" | "submissions">(
    "build",
  );
  ...
  const [subFilterField, setSubFilterField] = useState("all");
  ...
  const updateField = (field: FormField) => {
    if (!template) return;
    updateTemplate({
      fields: template.fields.map((f) => (f.id === field.id ? field : f)),
    });
  };
  ...
  const saveEdit = useCallback(
    (subId: string, fieldId: string, value: string) => {
      setSubmissions((prev) =>
        prev.map((s) => {
    ...
    (file: File) => {
      ...
      reader.onload = async (ev) => {
        ...
        if (headers.length === 0) {
          setCsvLoading(false);
          setCsvError(
            "No columns found — make sure the first row contains headers.",
          );
          return;
        }
    ...
    <div className="flex h-dvh bg-[#0a0a0a] text-[#e0e0e0] font-mono overflow-hidden">
      ...
      <div
        ...
        style={
          isMobile
            ? {
                width: sidebarOpen ? "min(228px, 85vw)" : 0,
              ...
              <div className="min-w-0 flex-1">
                <p
                  className={cn(
                    ...
                    activeId === t.id
                      ? "text-[#4D96D9] font-bold"
                      : "text-[#d0d0d0] font-normal",
                  )}
                >
                  {t.name}
                </p>
                ...
                {(["build", "preview", "submissions"] as const).map((m) => (
                  <button
                    ...
                    className={cn(
                      ...
                      isMobile
                        ? "px-2.5 py-1.5 gap-0"
                        : "px-2.75 py-1.25 gap-1.25",
                      mode === m
                        ? "bg-[rgba(255,255,255,0.09)] text-[#e0e0e0] font-semibold"
                        : "bg-transparent text-[rgba(255,255,255,0.35)] font-normal",
                    )}
                  >
              ...
              <button
                ...
                className={cn(
                  ...
                  dirty
                    ...
                    : "bg-[rgba(255,255,255,0.05)] border-[rgba(255,255,255,0.14)] text-[rgba(255,255,255,0.25)] cursor-not-allowed",
                )}
              >
                {saving ? "Saving…" : "Save"}
              </button>
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div
            className={cn(
              ...
              isMobile ? "px-3.5 py-4" : "px-9 py-7",
            )}
          >
            {!template ? (
                ...
                <div className="flex flex-col gap-1.25">
                  ...
                  {template.fields.map((field, idx) => (
                    <div key={field.id}>
                      <FieldCard
                        ...
                        onDelete={() =>
                          setConfirmDelete({
                            message: `Delete the "${field.label || field.type}" field?`,
                  ...
                  {template.fields.length === 0 && (
                    <div className="text-center py-14 text-[rgba(255,255,255,0.12)] text-[13px] border border-dashed border-[rgba(255,255,255,0.06)] rounded-xl mt-1">
                      Click{" "}
                      <strong className="text-[rgba(255,255,255,0.25)]">
                        Add field
                      </strong>{" "}
                      above to start building
                    </div>
                  )}
                </div>
                ...
                const filteredSubmissions = query
                  ? submissions.filter((sub) => {
                      const fieldsToSearch =
                        subFilterField === "all"
                          ? dataFields
                          : dataFields.filter((f) => f.id === subFilterField);
                        ...
                        {(["table", "card"] as const).map((v) => (
                          <button
                            ...
                            className={cn(
                              ...
                              subLayout === v
                                ? "bg-[rgba(255,255,255,0.1)] text-[#e0e0e0] font-bold"
                                : "bg-transparent text-[rgba(255,255,255,0.3)] font-normal",
                            )}
                          >
                            {v === "table" ? (
                              <svg
                      ...
                      <div className="flex items-center gap-2 mb-4">
                        <div className="flex-1 relative">
                          <svg
                            ...
                            fill="none" stroke="rgba(255,255,255,0.3)" strokeWidth="1.4" strokeLinecap="round"
                          >
                          ...
                          <input
                            ...
                            className="w-full bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.1)] rounded-lg pl-7 pr-7 py-1.5 font-mono text-[11px] text-[#d0d0d0] placeholder-[rgba(255,255,255,0.2)] outline-none focus:border-[rgba(255,255,255,0.2)]"
                          />
                          {subSearch && (
                            <button
                              onClick={() => {
                              ...
                              className="absolute right-2 top-1/2 -translate-y-1/2 text-[rgba(255,255,255,0.3)] bg-transparent border-none cursor-pointer leading-none"
                              style={{ fontSize: 14 }}
                            >×</button>
                          )}
                        </div>
                        {dataFields.length > 1 && (
                          <select
                            value={subFilterField}
                            onChange={(e) => {
                              setSubFilterField(e.target.value);
                              setCardPage(0);
                              setTableScrollTop(0);
                              if (tableScrollRef.current) tableScrollRef.current.scrollTop = 0;
                            }}
                            className="bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.1)] rounded-lg px-2.5 py-1.5 font-mono text-[11px] text-[rgba(255,255,255,0.5)] outline-none cursor-pointer"
                          >
                    ...
                    {submissions.length === 0 ? (
                      ...
                      (() => {
                        ...
                        const visibleEnd = Math.min(
                          ...
                          Math.ceil(
                            (tableScrollTop + TABLE_VIEWPORT_HEIGHT) /
                              ROW_HEIGHT,
                          ) + overscan,
                        );
                        const visibleRows = filteredSubmissions.slice(
                          visibleStart,
                          visibleEnd,
                        );
                        const topSpacer = visibleStart * ROW_HEIGHT;
                        const bottomSpacer =
                          (filteredSubmissions.length - visibleEnd) * ROW_HEIGHT;
                        return (
                          <div
                            ref={tableScrollRef}
                          ...
                          >
                            <table className="w-full border-collapse font-mono">
                              <thead className="sticky top-0 z-10 bg-[#0f0f0f]">
                                <tr>
                                  <th className={cn(thStyle, "w-32.5")}>
                                    Submitted
                                  </th>
                                  {dataFields.map((f) => (
                                    <th key={f.id} className={thStyle}>
                                      {f.label}
                                    </th>
                                  ))}
                              ...
                              <tbody>
                                {topSpacer > 0 && (
                                  <tr style={{ height: topSpacer }}>
                                    <td colSpan={dataFields.length + 2} />
                                  </tr>
                                )}
                                        ...
                                        return (
                                          <td
                                            ...
                                            className={cn(
                                              ...
                                              "cursor-text",
                                              isEditing &&
                                                "bg-[rgba(77,150,217,0.27)]! overflow-visible! whitespace-normal",
                                            )}
                                            onClick={() => {
                                              if (!isEditing) {
                                                setEditingCell({
                                          ...
                                          >
                                            {isEditing ? (
                                              <div className="flex flex-col gap-1.5">
                                                ...
                                                <div className="flex gap-1">
                                                  ...
                                                  <button
                                                    ...
                                                    className="px-1.5 py-0.5 bg-[rgba(255,255,255,0.14)] border-none rounded text-[rgba(255,255,255,0.45)] font-mono text-[10px] cursor-pointer leading-none"
                                                  >
                                                    Cancel
                                                  </button>
                                                </div>
                                              ...
                                              <span className="text-[#c8c8c8] text-[11px]">
                      ...
                      (() => {
                        const totalPages = Math.ceil(
                          filteredSubmissions.length / CARDS_PER_PAGE,
                        );
                          ...
                          <div className="flex flex-col gap-4">
                            <div className="grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-3">
                              {pageSubmissions.map((sub) => (
                                <div
                                  key={sub.id}
                                  className="bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.14)] rounded-xl p-4 flex flex-col gap-2.5 transition-colors duration-130"
                                  onMouseEnter={(e) =>
                                    ((
              ...
              <div className="max-w-150 mx-auto">
                {previewSubmitted ? (
                  <div className="bg-[rgba(90,173,107,0.06)] border border-[rgba(90,173,107,0.18)] rounded-[14px] p-10 text-center">
                    ...
                    <pre className="bg-[rgba(0,0,0,0.4)] border border-[rgba(255,255,255,0.06)] rounded-[9px] p-4.5 text-left text-[#c0c0c0] font-mono text-[11px] overflow-x-auto leading-[1.6]">
                      {JSON.stringify(
                        Object.fromEntries(
                          template.fields
                            .filter(
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-4.5">
                      {template.fields.map((field) => {
                        const isLayout =
                          FIELD_META[field.type].category === "Layout";
                        ...
                        return (
                          <div
                            key={field.id}
                            style={{ flex: `1 0 ${flexBasis}`, minWidth: 0 }}
                          >
                            <PreviewField
                              field={field}
                              value={
      ...
      {csvError && (
        ...
        >
          <div
            ...
            className="bg-[#111] border border-[rgba(255,255,255,0.1)] rounded-[12px] p-6 w-80 shadow-[0_24px_60px_rgba(0,0,0,0.8)]"
          >
            <p className="text-[#E85D7A] font-mono text-[13px] font-bold mb-1.5">
              Upload failed
            </p>
            <p className="text-[rgba(255,255,255,0.4)] font-mono text-[11px] leading-[1.6] mb-5">
              {csvError}
